...
- Раньше модуль всегда ставил allowlist-файрвол (`policy drop`: всё, кроме SSH/сервисных/
  ...
  - **`open`** — вся защита (flag-drop/анти-спуф/SYN+UDP-flood/SSH-бан/CrowdSec) активна,
    но не перечисленные порты **не** блокируются (`policy accept` + catch-all accept).
    Анти-скан автобан при этом не ставится («закрытых» портов нет — meter банил бы
    легитимные коннекты к inbound-портам); node-port правила тоже (на 3x-ui `NODE_PORT`
    может оказаться чьим-то inbound'ом — скрытый drop на нём кошмарен при отладке);
